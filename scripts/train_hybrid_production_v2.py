#!/usr/bin/env python3
"""Production-Grade Hybrid Predictive Training v2.

IMPROVEMENTS over v1:
- Hybridized gradient prediction (4 methods with adaptive weighting)
- Dynamic prediction horizon based on confidence
- Bias-corrected learning rate scaling
- Error-weighted correction steps
- Better confidence calibration (gradient alignment)
...
6. Confidence Metrics:
   - Multi-signal confidence (alignment, stability, agreement)
   - Dynamic thresholds based on prediction accuracy
   - Bias-corrected learning rate scaling

Usage:
    python scripts/train_hybrid_production_v2.py --model 100M --max-steps 5000
"""
...
# Import hybrid gradient predictor
from hybrid_gradient_predictor import (
    HybridGradientPredictor,
    compute_dynamic_horizon,
    compute_adaptive_lr_scale,
    compute_correction_steps,
)
...
@dataclass
class PhaseConfig:
    """Phase configuration with v2 improvements."""
    warmup_steps: int = 500
    min_full_steps: int = 25  # Reduced: less full training needed with better predictor
    base_predict_horizon: int = 12  # Base horizon (scaled by confidence)
    max_predict_horizon: int = 30  # Max horizon when confidence is very high
    min_predict_horizon: int = 2   # Min horizon when confidence is low
    min_correction_steps: int = 3  # Min correction steps
    max_correction_steps: int = 25 # Max correction steps (for large errors)
    correction_error_multiplier: float = 15.0  # Multiplier for error-based correction
    confidence_threshold: float = 0.85  # Raised: be more conservative entering PREDICT
    min_phase_duration: int = 8  # Minimum steps before phase change
    # Adaptive LR scaling parameters
    base_lr_scale: float = 0.8  # Base LR scale in predict phase
    min_lr_scale: float = 0.4   # Minimum LR scale
    max_lr_scale: float = 1.0   # Maximum LR scale (can use full LR if confident)
    divergence: DivergenceConfig = field(default_factory=DivergenceConfig)


class PhaseController:
    """Phase controller v2 with hybrid prediction and adaptive parameters."""
    ...
    def __init__(self, config: PhaseConfig, memory_budget_mb: float = 512.0):
        ...
        self.current_dynamic_horizon = config.base_predict_horizon
        self.correct_counter = 0
        self.target_correction_steps = config.min_correction_steps
        ...
        # V2: Use HybridGradientPredictor instead of simple GradientPredictor
        self.predictor = HybridGradientPredictor(
        ...
        # Track prediction errors for error-weighted corrections
        self.recent_prediction_errors: deque[float] = deque(maxlen=10)
        self.recent_prediction_bias: float = 0.0  # Mean(predicted - actual)
        ...
        self.metrics = {
            ...
            "recoveries": 0,
            "method_weights_history": [],  # Track which methods are winning
        }
    ...
    def _update_phase(self):
        """Phase transition logic v2 with dynamic horizon and error-weighted corrections."""
        if self.phase == Phase.WARMUP:
            ...
            if (self.phase_step >= self.config.min_full_steps and
                ...
                # V2: Calculate dynamic horizon based on confidence
                self.current_dynamic_horizon = compute_dynamic_horizon(
                    base_horizon=self.config.base_predict_horizon,
                    confidence=self.predictor.confidence,
                    min_horizon=self.config.min_predict_horizon,
                    max_horizon=self.config.max_predict_horizon,
                )
                self.transition_to(Phase.PREDICT, f"Conf={self.predictor.confidence:.2f}, Horizon={self.current_dynamic_horizon}")

        elif self.phase == Phase.PREDICT:
            self.predict_horizon += 1

            # V2: Use dynamic horizon instead of fixed max
            if self.predict_horizon >= self.current_dynamic_horizon:
                # Calculate error-weighted correction steps
                mean_error = (
                    statistics.mean(self.recent_prediction_errors)
                    if self.recent_prediction_errors else 0.1
                )
                self.target_correction_steps = compute_correction_steps(
                    prediction_error=mean_error,
                    prediction_horizon=self.predict_horizon,
                    min_steps=self.config.min_correction_steps,
                    max_steps=self.config.max_correction_steps,
                    error_multiplier=self.config.correction_error_multiplier,
                )
                self.transition_to(Phase.CORRECT, f"Horizon {self.predict_horizon}, Corrections={self.target_correction_steps}")
            elif self.predictor.confidence < self.config.confidence_threshold * 0.6:
                # V2: Slightly lower threshold (0.6 vs 0.7) since we're more confident
                self.target_correction_steps = self.config.max_correction_steps // 2
            ...
            # V2: Use error-weighted target instead of fixed count
            if self.correct_counter >= self.target_correction_steps:
                self.transition_to(Phase.FULL, f"Corrections complete ({self.correct_counter} steps)")
    ...
    def reset_for_recovery(self):
        ...
        # V2: Reset prediction tracking
        self.recent_prediction_errors.clear()
        self.recent_prediction_bias = 0.0
        self.current_dynamic_horizon = self.config.base_predict_horizon
        self.target_correction_steps = self.config.min_correction_steps
...
def train_hybrid_production(config: HybridTrainerConfig) -> dict[str, Any]:
    ...
    for step in range(1, config.max_steps + 1):
        ...
        if controller.should_use_full_gradient():
            ...
            # PREDICT phase v2 with hybrid prediction and adaptive LR
            optimizer.zero_grad()

            # V2: Use dynamic horizon from controller
            horizon = controller.predict_horizon if controller.predict_horizon > 0 else 1
            predicted_grads = controller.predictor.predict(horizon)
            predicted_loss = controller.predictor.predict_loss(loss_value, horizon)
            ...
            if controller.stats.is_ready:
                ...
                if loss_zscore > 2.5:  # V2: Slightly higher threshold (2.5 vs 2.0)
                    loss_spike = True
            ...
            if predicted_grads and not loss_spike:
                ...
                # V2: Tighter threshold based on confidence
                max_allowed = baseline * (2.0 + 4.0 * controller.predictor.confidence)

                if max_pred_norm < max_allowed:
                    apply_gradients(model, predicted_grads)

                    # V2: Use adaptive LR scaling with bias correction
                    lr_scale = compute_adaptive_lr_scale(
                        base_scale=config.phase.base_lr_scale,
                        confidence=controller.predictor.confidence,
                        recent_bias=controller.recent_prediction_bias,
                        min_scale=config.phase.min_lr_scale,
                        max_scale=config.phase.max_lr_scale,
                    )
                    for pg in optimizer.param_groups:
                    ...
                    # V2: Track prediction error for future corrections
                    if predicted_loss is not None and math.isfinite(predicted_loss):
                        error = abs(predicted_loss - loss_value) / max(loss_value, 1e-8)
                        controller.recent_prediction_errors.append(error)
                        bias = (predicted_loss - loss_value) / max(loss_value, 1e-8)
                        # EMA update of bias
                        controller.recent_prediction_bias = (
                            0.9 * controller.recent_prediction_bias + 0.1 * bias
                        )
                else:
    ...
    # Final metrics - comprehensive v2 tracking
    ...
    backward_reduction = (1 - backward_passes / max(forward_passes, 1)) * 100

    # Tokens per backward pass (efficiency metric)
    tokens_per_backward = total_tokens / max(backward_passes, 1)
    tokens_per_second = total_tokens / max(total_time, 1)

    # Prediction accuracy (from controller's error tracking)
    mean_prediction_error = (
        statistics.mean(controller.recent_prediction_errors)
        if controller.recent_prediction_errors else 0.0
    )
    prediction_bias = controller.recent_prediction_bias

    # Method weights from hybrid predictor (shows which methods are working best)
    method_weights = controller.predictor.get_method_weights()
    method_errors = controller.predictor.get_method_errors()

    # Phase transition summary
    phase_transitions = controller.metrics["phase_transitions"]
    phase_counts = {"WARMUP": 0, "FULL": 0, "PREDICT": 0, "CORRECT": 0}
    for pt in phase_transitions:
        phase_counts[pt["to"]] = phase_counts.get(pt["to"], 0) + 1

    final_metrics = {
        "model_size": config.model_size,
        "methodology": "hybrid_predictive_v2",
        "version": "2.0",
        ...
        # Forward/Backward reduction metrics
        "backward_passes": backward_passes,
        "forward_passes": forward_passes,
        "predictions_used": predictions_used,
        "backward_reduction_percent": backward_reduction,
        "forward_efficiency": backward_reduction,  # Alias for clarity

        # Efficacy metrics
        "tokens_per_second": tokens_per_second,
        "tokens_per_backward_pass": tokens_per_backward,
        "effective_throughput_multiplier": forward_passes / max(backward_passes, 1),
        ...
        # Hybrid predictor method weights (which methods are most accurate)
        ...
        "phase_entry_counts": phase_counts,
    ...
    # Summary
    print(f"\n{'=' * 70}")
    print("Training Complete! (Hybrid Predictive v2)")
    print(f"{'=' * 70}")
    ...
    print()
    print("=== Forward/Backward Reduction ===")
    print(f"  Forward passes: {final_metrics['forward_passes']:,}")
    print(f"  Backward passes: {final_metrics['backward_passes']:,}")
    print(f"  Predictions used: {final_metrics['predictions_used']:,}")
    print(f"  Backward reduction: {final_metrics['backward_reduction_percent']:.1f}%")
    print(f"  Effective throughput: {final_metrics['effective_throughput_multiplier']:.2f}x")
    print()
    print("=== Efficacy Metrics ===")
    print(f"  Tokens/second: {final_metrics['tokens_per_second']:,.0f}")
    print(f"  Tokens/backward: {final_metrics['tokens_per_backward_pass']:,.0f}")
    print(f"  Mean prediction error: {final_metrics['mean_prediction_error']:.3f}")
    print(f"  Prediction bias: {final_metrics['prediction_bias']:.4f}")
    print(f"  Final confidence: {final_metrics['final_confidence']:.2f}")
    print()
    print("=== Hybrid Predictor Method Weights ===")
    for method, weight in final_metrics['method_weights'].items():
        error = final_metrics['method_errors'].get(method, 0)
        print(f"  {method}: weight={weight:.2f}, error={error:.3f}")
    print()
    print("=== Stability ===")
    print(f"  Recoveries: {final_metrics['recoveries']}")
    print(f"  Divergence events: {final_metrics['divergence_events']}")
    print(f"  Phase transitions: {final_metrics['phase_transition_count']}")
...
def main():
    parser = argparse.ArgumentParser(description="Production Hybrid Predictive Training v2")
    ...
    # Phase configuration v2
    parser.add_argument("--warmup-steps", type=int, default=500)
    parser.add_argument("--min-full-steps", type=int, default=25)  # V2: reduced
    parser.add_argument("--base-predict-horizon", type=int, default=12)  # V2: new
    parser.add_argument("--max-predict-horizon", type=int, default=30)  # V2: increased
    parser.add_argument("--confidence-threshold", type=float, default=0.85)  # V2: raised
    ...
    # V2: Updated PhaseConfig construction with new parameters
    phase_config = PhaseConfig(
        warmup_steps=args.warmup_steps,
        min_full_steps=args.min_full_steps,
        base_predict_horizon=args.base_predict_horizon,
